...
const ProductAdminPage: FC<Props> = ({ product }) => {
	...
	const onChangeSize = (size: string) => {
		...
		const orderedSizes = newSizes.sort(
			(a, b) => order.indexOf(a) - order.indexOf(b)
		);

		setValue("sizes", orderedSizes, { shouldValidate: true });
	};
	...
	const onNewTag = () => {
		...
		// With push we don't need setValue cause currentTags is a reference
	...
	const onSubmit = async (formData: FormData) => {
		...
		try {
			const { data } = await tesloApi({
				url: "/admin/products",
				method: formData._id ? "PUT" : "POST", // If _id is null, it's a new product
				data: formData,
			});
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={12} sm={6}>
						...
						<TextField
							...
							onKeyUp={({ key }) => (key === " " ? onNewTag() : undefined)}
						/>
